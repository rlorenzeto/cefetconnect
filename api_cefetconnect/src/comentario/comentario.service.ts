...
export class ComentarioService {
  ...
  async create(idPost: string, idUsuario: number, createComentarioDto: CreateComentarioDto) {
    ...
    // Incrementa contador apenas no primeiro comentário do usuário no post
    if (comentariosDoUsuarioNoPost === 1) {
      // Incrementa contador do usuário que comentou (+1 por post comentado)
  ...
  async remove(id: string, idUsuario: number) {
    ...
    // Decrementa contador apenas se for o único comentário do usuário no post
  ...
  async curtirComentario(idComentario: string, idUsuario: number) {
    ...
    const jaCurtiuAlgumComentarioNestePost = curtidasNoPost[0]?.total > 0;
    ...
    // Incrementa contador apenas na primeira curtida de comentário no post
    if (!jaCurtiuAlgumComentarioNestePost) {
      await this.interacaoService.incrementarContador(idUsuario, 1);
    }
  ...
  async descurtirComentario(idComentario: string, idUsuario: number) {
    ...
    // Conta quantas curtidas o usuário tem em comentários deste post (incluindo a atual)
    ...
    // Decrementa contador apenas se era a última curtida de comentário no post
