...
export class PinService {
  ...
  async importar(dto: ImportarPinsDto, idUsuario: number) {
    ...
    // Incrementa contador do usuário (+5 por cada pin importado do Gradment)
  ...
  async remove(idPin: string, idUsuario: number) {
    ...
    // Decrementa contador se o pin foi importado do Gradment (-5 pontos)
    if (associacao.origem === OrigemPin.GRADMENT) {
      await this.interacaoService.decrementarContador(idUsuario, 5);
    }
