...
export class GradmentService {
  ...
  /**
   * Busca eixos/matérias aprovadas usando o token de integração do usuário
   * Endpoint: GET /integracao/materias-aprovadas
   * Header: X-Integration-Token
   */
  async obterEixosCompletados(tokenIntegracao: string): Promise<GradmentMateria[]> {
    if (!this.isConfigurado()) return [];

    try {
      const response = await fetch(`${this.baseUrl}/integracao/materias-aprovadas`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        this.logger.warn(`[Gradment] Falha ao obter eixos: HTTP ${response.status}`);
        return [];
      }
      ...
      this.logger.error(`[Gradment] Erro de conexão em obterEixosCompletados: ${e}`);
      return [];
    }
  }
}
